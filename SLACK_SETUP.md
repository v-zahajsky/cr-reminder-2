...
3. Give it a name (e.g. `Code Review Reminder`) and pick the target workspace.
4. Click **Create App**.
...
The actor tags PR authors using their Slack user ID. For each developer you want tagged:
...
4. Put the mapping into the actor input `userMapping` as JSON:
   ...
   The key is the **GitHub login** (visible on the PR author's profile URL, e.g. `github.com/radimkvet`).

### What happens if a mapping is missing?

The actor still includes the PR in the message but prints the author as plaintext `@login` (no real tag, no notification) and logs a warning. Add the missing user to `userMapping` before the next run.

## 7. Verify end-to-end

1. In the actor input, set `mode: "all"` and `sendEmptyReport: true`.
